      ...
      if (p.proposal_id == id && !votado) {
        if (!p.decided) {
          ...
          p.votes.forEach(vote => {
            log("foreach ejecutandose");
            if (vote.id == predecessorAccountId()) {
              log("encontre un voto repetido");
              votado = true;
              idData = vote.id;
            }
          });
          ...
          log(`${idData} had already voted`);
        } else {
          log('Voting had already been completed');
        }
        votado = true;
      }
    }
...
function vote_for() {
  ...
  const _result = _contract.vote_for(_args);
...
function set_proposal() {
  ...
  const _args = VotingNear._getArgs();
  const _result = _contract.set_proposal(_args);
  VotingNear._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(VotingNear._serialize(_result, true));
}
function get_proposal() {
  ...
  const _args = VotingNear._getArgs();
  const _result = _contract.get_proposal(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(VotingNear._serialize(_result, true));
}
