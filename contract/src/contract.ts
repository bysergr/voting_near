...
class VotingNear {
  ...
  set_proposal({ text }: { text: string }): void {
    ...
    const proposal: PostedProposal = new PostedProposal(
  ...
  set_winner({ id }: { id: number }): boolean {
    ...
    for (let i = 0; i < propuestas.length; i++) {
      near.log("buscando elemento");
      if (
        propuestas.get(i).proposal_id == id &&
        propuestas.get(i).sender == near.predecessorAccountId()
      ) {
    ...
    if (state == false) {
      near.log(
        "No eres la persona que creo la propuesta no puedes finalizarla"
      );
    }
    this.proposals = propuestas;
    return state;
